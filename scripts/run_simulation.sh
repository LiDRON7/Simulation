...
echo "[SIM] Using display: $DISPLAY"
...
if [ ! -f "/px4/Tools/simulation/gz/worlds/${WORLD_NAME}.sdf" ]; then
...
# ── Launch PX4 SITL ───────────────────────────────────────────
echo "[SIM] Launching PX4 SITL + Gazebo server (headless)..."
cd /px4
HEADLESS=1 make px4_sitl gz_x500_${WORLD_NAME}
# Note: no & here — we wait for make to confirm it started successfully
# before launching the GUI and bridge. make exits once PX4 is running.
PX4_PID=$!

echo "[SIM] PX4 SITL launched, waiting 15s for Gazebo server to settle..."
sleep 15

# ── Launch Gazebo GUI ─────────────────────────────────────────
echo "[SIM] Launching Gazebo GUI on display $DISPLAY..."
gz gui &
GZ_GUI_PID=$!
...
echo "[SIM] GZ GUI  PID=$GZ_GUI_PID"
